    ...
    case 'obj':
        ...
        objLoader.load(
            ...
            (object) => {
                ...
                setCamera(boxSize)
                guiInitializer(absMaterial, object)
            },
            (xhr) => {
                console.log((xhr.loaded / xhr.total) * 100 + '% loaded')
            },
...
function renderObject(fisicObj: any) {
    scene.add(fisicObj)
    fisicObj.position.set(0, 0, 0)
    fisicObj.castShadow = true
    fisicObj.receiveShadow = true
    boxSize = getBoxSize(fisicObj)
    setCamera(boxSize)
}
function guiInitializer(material: any, objectTD: any) {
    const options = {
        wireframe: false,
        object: 0x1111111,
    ...
    colorFolder.addColor(options, 'object').onChange(col => { absMaterial.color = new THREE.Color(col) })
}
